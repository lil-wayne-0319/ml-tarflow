...
def eval_loc_auroc(loc_auroc_obs, epoch, gt_mask, anomaly_score_map):
    ...
    # assert gt_mask.shape == anomaly_score_map.shape
    # c, h, w = gt_mask.shape
    # min_num = 1000
    # if c > min_num:
    #     loc_auroc_all = []
    #     for i in range(c // min_num + 1):
    #         gt_mask_split = gt_mask[i*min_num : (i+1)*min_num, :, :]
    #         anomaly_score_map_split = anomaly_score_map[i*min_num : (i+1)*min_num, :, :]
    #         loc_auroc_split = roc_auc_score(gt_mask_split.flatten(), anomaly_score_map_split.flatten()) * 100
    #         loc_auroc_all.append(loc_auroc_split)
    #     loc_auroc_mean = np.mean(loc_auroc_all)
    #
    #     best = loc_auroc_obs.update(loc_auroc_mean, epoch)
    #     return loc_auroc_mean, best
    # else:
